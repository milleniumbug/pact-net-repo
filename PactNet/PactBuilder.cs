...
namespace PactNet
{
    public class PactBuilderOld : IPactBuilder
    {
        ...
        public PactBuilderOld()
            : this(new PactConfig())
        {
        }

        public PactBuilderOld(PactConfig config)
            : this((port, enableSsl, consumerName, providerName, host, jsonSerializerSettings, sslCert, sslKey) =>
                new MockProviderServiceOld(port, enableSsl, consumerName, providerName, config, host,
        ...
        {
            (_mockProviderService as MockProviderServiceOld)?.SendAdminHttpRequest(HttpVerb.Post, Constants.PactPath);
        }
    }
}